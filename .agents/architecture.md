...
## 何时更新
- 当系统模块边界、目录结构或职责划分发生变化时，必须更新本文件。
- 当前后端数据流、同步策略、认证链路、去重策略或存储策略发生变化时，必须更新本文件。
- 当新增或替换关键基础设施、数据库方案或服务依赖时，必须更新本文件。
...
- **用户表**: 存储雪球和 Twitter 用户信息。雪球首页 feed 的帖子保存前会自动 upsert 用户，确保帖子能正常显示头像和用户名。
...
4. **查重**: 存储层按来源内 ID 防止重复写入；前端聚合层按 `source:id` 防止跨来源冲突。
