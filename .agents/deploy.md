...
## 何时更新
- 当部署平台、构建命令、发布命令或上线顺序发生变化时，必须更新本文件。
- 当生产环境变量、数据库初始化流程、云端凭证配置方式或反向代理路径发生变化时，必须更新本文件。
- 当 Render、Vercel、Supabase 的接入方式或限制说明发生变化时，必须更新本文件。
...
## 相关引用
- **架构流向**: [architecture.md](@file:.agents/architecture.md)
- **本地配置**: [setup.md](@file:.agents/setup.md)
