...
### saveXueqiuPosts 的行为说明

- 在批量保存雪球帖子前，会先从帖子中提取 `post.user` 信息并对 `xueqiu_users` 执行 `upsert(onConflict: 'user_id')`。
- 该步骤用于保障“首页 feed 帖子”场景下用户资料存在，避免帖子展示时缺少头像/用户名。
- 用户 upsert 失败不会中断帖子保存流程，系统会继续写入 `xueqiu_posts`。
